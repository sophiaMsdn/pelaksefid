...
public class AdvertiseRepositoryCustomImpl implements AdvertiseRepositoryCustom {
    ...
    private String getAdvertiseQueryWithFilter(AdvertiseFilter filter) {
        String filterQuery = (filter.getTypeIds() != null ? " and ct.id in (:typeIds) " : "")
                ...
                + (filter.getFuelIds() != null ? " and ct.id in (select ct.id from ps_car_type_fuels f join ps_car_types ct on f.ps_car_type_id = ct.id join ps_car_fuels fu on f.ps_fule_id = fu.id where fu.id in (:fuelIds))" : "");
        ...
        String query = " select "
                ...
                + " join ps_accounts ac on a.ps_account_id = ac.id"
                ...
                + " left join ps_regions cty on a.ps_city_id = cty.id"
    ...
    @Override
    public Page<AdvertiseModel> getAgencyAdvertisesByFilter(AdvertiseFilter filter, Pageable pageRequest) {

        try {
            String query = getAgencyAdvertiseQueryWithFilter(filter);
            String queryForCounting = getAgencyAdvertiseQueryWithFilterForCounting(filter);
            ...
            if (!ValidationUtil.isEmpty(filter.getAgencyId())) {
                sqlQuery.setParameter("agencyId", filter.getAgencyId());
                sqlQueryForCounting.setParameter("agencyId", filter.getAgencyId());
            }
            Integer from = pageRequest.getPageNumber() * pageRequest.getPageSize() + 1;
            Integer to = from + pageRequest.getPageSize() - 1;
            sqlQuery.setParameter("from", String.valueOf(from));
    ...
    private String getAgencyAdvertiseQueryWithFilter(AdvertiseFilter filter) {
        String filterQuery = (filter.getTypeIds() != null ? " and ct.id in (:typeIds) " : "")
                + (filter.getBrandIds() != null ? " and b.id in (:brandIds) " : "")
                + (filter.getModelIds() != null ? " and cm.id in (:modelIds) " : "")
                + (!ValidationUtil.isEmpty(filter.getAccountId()) ? " and ac.id = :accountId " : "")
                + (!ValidationUtil.isEmpty(filter.getAgencyId()) ? " and ag.id = :agencyId " : "");

        String sort;
        if (ValidationUtil.isEmpty(filter.getSortType())) {
            sort = " row_number() over (order by a.id desc) rn";
        } else {
            switch (filter.getSortType()) {
                case "DATE_ASC":
                    sort = " row_number() over (order by a.update_date asc) rn";
                    break;
                case "DATE_DESC":
                    sort = " row_number() over (order by a.update_date desc) rn";
                    break;
                case "PRICE_ASC":
                    sort = " row_number() over (order by a.price asc) rn";
                    break;
                case "PRICE_DESC":
                    sort = " row_number() over (order by a.price desc) rn";
                    break;
                case "MILEAGE_DESC":
                    sort = " row_number() over (order by c.mileage asc) rn";
                    break;
                default:
                    sort = " row_number() over (order by a.id desc) rn";
                    break;
        ...
        String query = " select "
                ...
                + " a.advertise_state advertiseState,"
                + " docs.info documents,"
                + " agencyDocs.info agencyDocuments,"
                + " sigmaDocs.info sigmaDocuments,"
                + " bs.description bodyStatus,"
                + " to_char(a.reg_date,'yyyy/mm/dd','nls_calendar=persian') registerDate,"
                + " to_char(a.update_date,'yyyy/mm/dd','nls_calendar=persian') updateDate,"
                ...
                + " from ps_advertises a"
                + " join ps_accounts ac on a.ps_account_id = ac.id"
                + " join ps_agencies ag on ac.id = ag.ps_account_id"
                + " join (select ad.ps_agency_id aId, rtrim(xmlagg(xmlelement(E, d.id || '*' || ad.AGENCY_DOC_TYPE, '|') .extract('//text()') order by d.id || '*' || ad.AGENCY_DOC_TYPE) .getclobval(), '|') info from ps_agency_documents ad left join ps_documents d on ad.document_id = d.id where ad.is_active = 1 and ad.is_alive = 1 and d.is_active = 1 and d.is_alive = 1 group by ad.ps_agency_id) agencyDocs on ag.id = agencyDocs.aId"
                + " left join ps_cars c on a.ps_car_id = c.id"
                + " left join ps_car_types ct on c.ps_car_type_id = ct.id"
                + " left join ps_car_models cm on ct.ps_car_model_id = cm.id"
                + " left join ps_brands b on cm.ps_brand_id = b.id"
    ...
    private String getAgencyAdvertiseQueryWithFilterForCounting(AdvertiseFilter filter) {
        String filterQuery = (filter.getTypeIds() != null ? " and ct.id in (:typeIds) " : "")
                + (filter.getBrandIds() != null ? " and b.id in (:brandIds) " : "")
                + (filter.getModelIds() != null ? " and cm.id in (:modelIds) " : "")
                + (!ValidationUtil.isEmpty(filter.getAccountId()) ? " and ac.id = :accountId " : "")
