...
public class AdvertiseDto extends BaseDto {
    ...
    private AgencyDocumentDto[] agencyDocuments;
    ...
    public Boolean isValid() {
        ...
        if (ValidationUtil.isEmpty(cityId)) {
            return false;
        }
    ...
    public void fromEntity(Base e) {
        ...
        regionDescription = ent.getRegion() != null ? ent.getRegion().getDescription() : "";
        ...
        cityId = ent.getCity().getId().toString();
        provinceId = ent.getCity().getParent().getId().toString();
    }
    ...
    public void fromModel(AdvertiseModel m) {
        ...
        provinceDescription = m.getProvince();
        try {
        ...
        try {
            if (m.getAgencyDocuments() != null) {
                String[] docs = m.getAgencyDocuments().split("\\|");
                AgencyDocumentDto[] ads = new AgencyDocumentDto[docs.length];
                int cntr = 0;
                for (String f : docs) {
                    String[] doc = f.split("\\*");
                    AgencyDocumentDto ad = new AgencyDocumentDto();
                    ad.setId(doc[0]);
                    ad.setDocType(doc[1]);
                    ads[cntr++] = ad;
                }
