...
public class AdvertiseService extends BaseService {
    ...
    public Advertise controlAndSave(AdvertiseReq req, HttpServletRequest request) {
        ...
        Optional<Region> city = regionRepository.findById(Long.valueOf(advertiseDto.getCityId()));
        if (!city.isPresent()) {
            return null;
        }
        advertise.setCity(city.get());
        if (!ValidationUtil.isEmpty(advertiseDto.getRegionId())) {
        ...
        if (advertiseDto.getDocuments() != null) {
    ...
    public ResultEnm validateBeforeUpdate(AdvertiseReq req) {
        if (req == null) {
            return ResultEnm.INVALID_INPUT;
        }
        if (req.getAdvertise() == null) {
    ...
    @Transactional
    public Advertise controlAndUpdate(AdvertiseReq req, HttpServletRequest request) {
        ...
        Optional<Advertise> optional = repository.findById(Long.valueOf(advertiseDto.getId()));
        if (!optional.isPresent()) {
            return null;
        }
        Advertise advertise = optional.get();
        advertise = (Advertise) advertiseDto.toEntity(advertise);
        advertise.setAdvertiseState(AdvertiseStateEnm.REGISTERED);
        advertise.setAccount(account);
        Optional<CarType> carType = carTypeRepository.findById(Long.valueOf(advertiseDto.getCarTypeId()));
        if (!carType.isPresent()) {
        ...
        for (AdvertiseDocument ad : oldDocs) {
            advertiseDocumentRepository.delete(ad);
        }
        for (DocumentDto d : advertiseDto.getDocuments()) {
            Document document = (Document) d.toEntity();
            document = documentRepository.save(document);
    ...
    public ResultEnm validateBeforeDeactive(AdvertiseReq req) {
        if (req == null) {
            return ResultEnm.INVALID_INPUT;
        }
        if (req.getAdvertise() == null) {
    ...
    @Transactional
    public Advertise controlAndDeactive(AdvertiseReq req, HttpServletRequest request) {
        if (req == null) {
            return null;
        }
        if (req.getAdvertise() == null) {
        ...
        advertise = (Advertise) logEntity(advertise, request);
        historyService.insertHistory(advertise,
                AdvertiseOperationEnm.DEACTIVE,
                user.getUsername(),
                user.getId().toString(),
                null,
                DateFormatUtil.getCurrentPersianDate(),
    ...
    public Page<AdvertiseModel> getAllAgencyAdvertisesByFilter(AdvertiseFilterReq req, HttpServletRequest request) {
        
        AdvertiseFilter filter = new AdvertiseFilter();
        filter.setAgencyId(req.getAgencyId());
        filter.setSortType(req.getSortType());
        if (!ValidationUtil.isEmpty(req.getBrandIds())) {
            String[] brandIds = req.getBrandIds().split(",");
        ...
        return repository.getAgencyAdvertisesByFilter(filter,
                PageRequest.of(page - 1, rows));
    }
    
    public Advertise getAdvertiseInfo(AdvertiseReq req, HttpServletRequest request) {
        if (req == null) {
            return null;
        }
        if (req.getAdvertise() == null) {
        ...
        Optional<Advertise> advertise = repository.findById(Long.valueOf(req.getAdvertise().getId()));
